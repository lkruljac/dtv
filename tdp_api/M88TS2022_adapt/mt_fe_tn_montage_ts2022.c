...
S32 SetTuner(U32 fPLL, U32 fSym, U16 lpfOffset, U8 gainHold)
{
	...
	mt_fe_print(" SetTuner fPLL = %d fSym = %d \n", fPLL, fSym);
	...
	{
		mt_fe_print(" SetTuner  gvTunerVersion == TUNER_M88TS2022\n");
		if(fREF == 1)
...
U8	GetSignalStrength(void)
{
	...
	if(gain > 8000)			strength = 0;								//0%           no signal or weak signal
	else if(gain > 5700)	strength = 0 + (8000 - gain)  * 3/100;	//0% - 60%     weak signal
	else if(gain > 3500)	strength = 60 + (5700 - gain) * 3 / 200;		//60% - 90%    normal signal
	else					strength = 90 + (3500 - gain) / 500;    	        //90% - 99%    strong signal
...
MT_FE_RET mt_fe_tn_set_freq_ts2022(U32 freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	g_tuner_freq_offset_KHz = SetTuner(freq_MHz, sym_rate_KSs, lpf_offset_KHz, 0);
