...
U8	g_ub_sum					 = 0;		/*User band sum, 0, 2, 4 or 8*/
...
{
	*p_version = 60205;		/* driver version number */
...
**
*******************************************************************************/
MT_FE_RET mt_fe_dmd_ds3k_init(void)
{
	U8 val;
	...
	if((g_dmd_id == MtFeDmdId_DS3002B) || (g_dmd_id == MtFeDmdId_DS3103))
	{
		...
		mt_fe_print("download_fw to m88ds310x_fm \n");
		ret = _mt_fe_dmd_ds3k_download_fw(m88ds310x_fm);
		if(ret != MtFeErr_Ok)
		{
...
MT_FE_RET mt_fe_dmd_ds3k_connect(U32 freq_MHz, U32 sym_rate_KSs, MT_FE_TYPE dvbs_type)
{
	...
	mt_fe_print(" Set freque started... \n");
...
#if ((MT_FE_TN_SUPPORT_TS2020 != 0)||(MT_FE_TN_SUPPORT_SHARP6306 != 0)||(MT_FE_TN_SUPPORT_SHARP7803 != 0)||(MT_FE_TN_SUPPORT_TS2022 != 0))
...
MT_FE_RET mt_fe_dmd_ds3k_get_lock_state(MT_FE_LOCK_STATE *p_state)
{
	...
	//mt_fe_print("tuner lock check 1 \n");
	*p_state = MtFeLockState_Waiting;
    ...
    //mt_fe_print("tuner lock check 2 \n");
	if (g_current_type == MtFeType_DvbS2)
	{
		if(g_dmd_id == MtFeDmdId_DS300X)
		{
			//mt_fe_print("tuner lock check 3 \n");
			_mt_fe_dmd_get_reg(0x0d, &tmp);
		...
		{
			//mt_fe_print("tuner lock check 4 \n");
	...
	{
		//mt_fe_print("tuner lock check 5 \n");
		_mt_fe_dmd_get_reg(0xd1, &tmp);
	...
	{
		//mt_fe_print("tuner lock check 6 \n");
		g_auto_tune_cnt = 1;
		...
		{
			//mt_fe_print("tuner lock check 7 \n");
			if((carrier_offset_KHz >= MT_FE_CARRIER_OFFSET_KHz)||
			   (carrier_offset_KHz <= - MT_FE_CARRIER_OFFSET_KHz))
			{
				_mt_fe_dmd_set_reg(0xb2, 0x01);
    ...
    //mt_fe_print("tuner lock check 8 \n");
	_mt_fe_dmd_get_reg(0xb3, &tmp);
	...
	{
		//mt_fe_print("tuner lock check 9 \n");
		if((tmp & 0x80) == 0x80)
		{
			*p_state = MtFeLockState_Unlocked;
			_mt_fe_dmd_set_reg(0xb2, 0x01);
		...
		if((tmp & 0x40) == 0x40)
		{
			//mt_fe_print("tuner lock check 10 \n");
    ...
    //mt_fe_print("tuner lock check 11  state = %d\n", *p_state);
	return MtFeErr_Ok;
}
...
MT_FE_RET _mt_fe_dmd_ds3k_set_demod(U32 sym_rate_KSs, MT_FE_TYPE type)
{
    ...
    mt_fe_print(" _mt_fe_dmd_ds3k_set_demod satrted \n");
...
#if ((MT_FE_TN_SUPPORT_SHARP6306 != 0) || (MT_FE_TN_SUPPORT_SHARP7803 != 0))
    ...
    mt_fe_print(" _mt_fe_dmd_ds3k_set_demod finished \n");
							  ...
							  U8 bs_times)
{
	...
	{
		mt_fe_print("_mt_fe_dmd_ds3k_bs_A  freq_KHz= %d \n",freq_KHz);
		...
		else
		{
			...
			#elif ((MT_FE_TN_SUPPORT_SHARP6306 != 0) || (MT_FE_TN_SUPPORT_SHARP7803 != 0))
			if(tmp_freq_KHz == 0)
			{
				freq_KHz += 30000;
			}
	...
	while((freq_KHz < end_freq_KHz)&&(!g_blindscan_cancel))
	{
		...
		if((spectrum_inverted == 1) && (g_dmd_id == MtFeDmdId_DS300X))
		{
			...
			if((tmp &= 0x1F) == 0)
			{
				...
				#if ((MT_FE_TN_SUPPORT_SHARP6306 == 0) && (MT_FE_TN_SUPPORT_SHARP7803 == 0))		// TS2022
									...
									U16 scanned_tp)
{
	...
	#if ((MT_FE_TN_SUPPORT_TS2020 != 0)||(MT_FE_TN_SUPPORT_TS2022 != 0) || (MT_FE_TN_SUPPORT_SHARP6306 != 0)|| (MT_FE_TN_SUPPORT_SHARP7803 != 0))
		if ((sym_rate_KSs < 5000) && (use_unicable_device == 0))
		{
...
U32 mt_fe_dmd_ds3k_unicable_FFT_detect(U32 startFreq, U32 *result_freq)		// Unit: KHz
{
	U32		freq = 0;
	U32		high_energy_average = 0;

	MT_FE_BOOL	b_analog_locked = MtFe_False, b_fftdone = MtFe_False;

	U8		tmp, cnt;
	U16		totaltpnum;

	U32		nSize = (512 - 192 + 2) * 2;	// sm_buf = 2, nSize = 644
	U8		buffer[644] = {0};				// sm = 2
	U32		nCount, i, j;
	U32		dwTmp;
	...
	*result_freq = 0;
	...
	_mt_fe_dmd_set_reg(0x91, 0x46);

	_mt_fe_dmd_set_reg(0x92, 0x01);
	...
	_mt_fe_dmd_set_reg(0x95, 0x64);


	tmp = (TUNER_SLIP_STEP*16 + 3);
	_mt_fe_dmd_set_reg(0x97, tmp);


	_mt_fe_dmd_set_reg(0x99, 0x1c);
	...
	if(g_dmd_id == MtFeDmdId_DS300X)
	{
		...
		_mt_fe_dmd_set_reg(0x3b, 0x00);
		_mt_fe_dmd_set_reg(0x3c, PSD_AGC);
	}
	else if((g_dmd_id == MtFeDmdId_DS3002B) || (g_dmd_id == MtFeDmdId_DS3103))
	{
		if(spectrum_inverted == 1)			// 0x4d, bit 1
	...
	//mt_fe_tn_set_tuner(freq, BLINDSCAN_SYMRATEKSs, 0);
	mt_fe_tn_set_tuner(freq + 3000, 20000, 0);

	if((spectrum_inverted == 1) && (g_dmd_id == MtFeDmdId_DS300X))
	{
		_mt_fe_dmd_ds3k_set_carrier_offset(3000 + (S32)(mt_fe_tn_get_offset()));
	}
	else
	{
		_mt_fe_dmd_ds3k_set_carrier_offset(- 3000 - (S32)(mt_fe_tn_get_offset()));
	...
	_mt_fe_dmd_get_reg(0x9a, &tmp);
	totaltpnum = (U16)(tmp & 0x1F);
	if((totaltpnum == 0) || ((tmp & 0xE0) != 0) || (b_analog_locked == MtFe_False) || (b_fftdone == MtFe_False))
	{
		*result_freq = 0;
	...
	_mt_fe_dmd_set_reg(0x9A, 0x40);

	for(j=0; j<nSize; j++)
	{
		_mt_fe_dmd_get_reg(0x9B, &tmp);
		buffer[j] = tmp;
	}
	...
	for(i=2*2; i<nSize; i+=2)
	{
		dwTmp = buffer[i];
		dwTmp = dwTmp + (buffer[i+1] * 256);

		PSDArray[nCount++] = dwTmp;
	}
	...
	high_energy_average = PSDArray[161];


	j = 161;
	for(i = 0; i < 11; i ++)
	{
		if(PSDArray[156 + i] > high_energy_average)
		{
			high_energy_average = PSDArray[156 + i];
			j = 156 + i;
		}
	}

	if(high_energy_average > PSD_THRESHOLD)	// 3000
	{
		if(spectrum_inverted == 1)
		{
			*result_freq = (S32)freq + (((S32)j - 161) * 96 * 1000 / 512);
		}
		else
		{
			*result_freq = (S32)freq - (((S32)j - 161) * 96 * 1000 / 512);
		...
		*result_freq += 500;
		*result_freq /= 1000;
		// return high_energy_average;
	}
	else
	{
		*result_freq = 0;
		// return high_energy_average;			// 0
	}
...
MT_FE_BOOL mt_fe_ds3k_unicable_CCI_detect(U32 startFreq, U32 *result_freq)	// Unit: KHz
{
	U8 i, cnt, reg_0xb3, reg_0x59, CCI_on_flag_cnt;
	S8 val_cci;
	...
	DB fCCIFreq = 0.0, fCCIAmp = 0.0, fTotalFO = 0.0;
	S32 CCIFreq = 0, CCIAmp = 0;

	S32 freq_offset, total_freq_offset;
	...
	*result_freq = 0;
	...
	_mt_fe_dmd_set_reg(0x3b, 0x00);
	_mt_fe_dmd_set_reg(0x3c, CCI_AGC);		// max 0x79
	if((g_dmd_id == MtFeDmdId_DS3002B) || (g_dmd_id == MtFeDmdId_DS3103))
	{
		_mt_fe_dmd_set_reg(0x00, 0x00);
	}
	...
	mt_fe_tn_set_tuner(startFreq, 8000, 0);
	...
	/*set demod registers*/
	_mt_fe_dmd_ds3k_set_demod(8000, g_current_type);
	...
	_mt_fe_dmd_ds3k_get_carrier_offset(&freq_offset);
	_mt_fe_dmd_ds3k_get_total_carrier_offset(&total_freq_offset);
...
MT_FE_UNICABLE_DEVICE	ub_list[MT_FE_UB_NUM] =
{
	{1210, FALSE, 0, 0},
	{1284, FALSE, 0, 0},
	{1400, FALSE, 0, 0},
...
MT_FE_RET mt_fe_dmd_ds3k_unicable_detect(BOOL need_init)
{
	U32 i, count = 0, cci_count = 0;
	U32 fft_result = 0;
	U32 fft_freq = 0;
	...
	for(i = 0; i < MT_FE_UB_NUM; i ++)
	{
		mt_fe_print(("\tDetecting [%4d] MHz ...\n", ub_list[i].freq_MHz));
		fft_result = mt_fe_dmd_ds3k_unicable_FFT_detect(ub_list[i].freq_MHz * 1000, &fft_freq);
		if((fft_result > PSD_THRESHOLD) && (fft_freq == ub_list[i].freq_MHz))		// 3000
		{
			#if NEED_CHECK_CCI
			MT_FE_BOOL cci_result[3] = {MtFe_False, MtFe_False, MtFe_False};
			U32 cci_freq[3] = {0, 0, 0};
			U32 j;

			cci_result[0] = mt_fe_ds3k_unicable_CCI_detect(ub_list[i].freq_MHz * 1000, &cci_freq[0]);
			cci_result[1] = mt_fe_ds3k_unicable_CCI_detect((ub_list[i].freq_MHz - 1) * 1000, &cci_freq[1]);
			cci_result[2] = mt_fe_ds3k_unicable_CCI_detect((ub_list[i].freq_MHz + 1) * 1000, &cci_freq[2]);

			cci_count = 0;
			for(j = 0; j < 3; j ++)
			{
				if((cci_freq[j] == fft_freq) && (cci_result[j] == MtFe_True))
				{
					cci_count ++;
				}
			}
			...
			if(cci_count > 1)
			{
				ub_list[i].UB_ok = TRUE;

				mt_fe_print(("\t[%4d] MHz Detectd! PSD Value = [%4d]\n", ub_list[i].freq_MHz, fft_result));
				mt_fe_print(("\tCCI Result = [%d]; CCI count = [%d]; CCI Freq = [%4d]\n", cci_result[0], cci_count, cci_freq[0]));
				count ++;
			}
			else
			{
				ub_list[i].UB_ok = FALSE;
			}
			#else
			ub_list[i].UB_ok = TRUE;
		...
		ub_list[i].result_MHz = (U16)fft_freq;
		ub_list[i].result_PSD = fft_result;
	}

	mt_fe_print(("\tTotal %d %s found!\n", count, (count == 1) ? "UB is" : "UBs are"));
