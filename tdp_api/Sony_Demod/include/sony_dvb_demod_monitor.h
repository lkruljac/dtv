...
/**
 @brief Monitor the signal quality (Version 2), expressed as a percentage.
        Identical to dvb_demod_monitor_Quality() but is able to apply smoothing to the 
        DVB-T2 MER read from the device (using ::dvb_demod_monitorT2_FilteredMER) 
        in order to provide a smoothed SQI value.
        Please refer to ::dvb_demod_monitor_Quality for more documentation on the 
        algorithm for SQI.
        
        <em>Important</em>: Call ::dvb_demod_monitorT2_InitMERFilter 
        after tuning to a new channel in order for the SQI to converge quickly;
        if using the MER filter.
    
 @param pDemod The demodulator instance.
 @param pMerFilter The MER filter to be used for the DVB-T2 MER measurement or 
                   NULL if not used. Passing NULL will essentially use the 
                   ::dvb_demod_monitor_Quality functionality.
 @param pQuality The quality as a percentage (0-100). Output.
