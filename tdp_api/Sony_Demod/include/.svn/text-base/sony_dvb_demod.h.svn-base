...
------------------------------------------------------------------------------*/
#define DVB_DEMOD_DVBT_ADDRESS              0x6c//0xD8    /**< DVB-T I2C address. */
#define DVB_DEMOD_DVBT2_ADDRESS            0x6c//0xD8    /**< DVB-T2 I2C address. Shared with DVB-T I2C address space. */
#define DVB_DEMOD_DVBC_ADDRESS              0x6e//0xDC    /**< DVB-C I2C address. */
...
//new version
#define DVB_DEMOD_DVBT2_LE_WAIT_SYR         3000    /**< DVB-T2 3000ms timeout for TS lock on Long Echo PP2 sequence before trialling SYR. */
...
*/
#define DVB_DEMOD_MAKE_IFFREQ_CONFIG_DVBT(iffreq) ((uint32_t)(((iffreq)/41.0)*16777216.0 + 0.5))

/**
 @brief Create the DVB-T2 fields for ::sony_dvb_demod_t::iffreq_config.
*/
#define DVB_DEMOD_MAKE_IFFREQ_CONFIG_DVBT2(iffreq) ((uint32_t)(((iffreq)/41.0)*16777216.0 + 0.5))
...
*/
#define DVB_DEMOD_MAKE_IFFREQ_CONFIG_DVBC(iffreq) ((uint16_t)(16384.0*(1.0-(iffreq)/41.0) + 0.5) & 0x3FFF)
...
*/
typedef struct {
    uint8_t IREG_SEQ_OSTATE;            /**< Debugging. */   
    uint8_t IREG_ARGD_STATE;            /**< Debugging. */  
    uint8_t IREG_COSNE_COK;             /**< Debugging. */  
...
typedef enum {
    ...
    DVB_TUNER_ASCOT2S,              /**< Sony ASCOT2S derived tuners. */
    DVB_TUNER_ASCOT2D              /**< Sony ASCOT2D derived tuners. */
...
typedef struct sony_dvb_demod_t {
    ...
    */
    uint8_t ioHiZ ;
    /**
     @brief DVB-T2 PP2 Long Echo sequence enable/disable flag. 
            Default: Disabled (0).
            Use ::dvb_demod_SetConfig to configure this flag.
    */
    uint8_t t2LongEchoPP2 ;
...
typedef enum {
    ...
    */
    ...
    /**
     @brief OREG_TSIF_SCR_ENABLE TS smoothing ON/OFF.
            *NOT recommended for normal use (testing/debug only).*
            Once TS smoothing is disabled, then DEMOD_CONFIG_TSIF_SDPR and 
            DEMOD_CONFIG_TS_AUTO_RATE_ENABLE will have no effect on the output 
            TS clock rate (data period).
            If DEMOD_CONFIG_PARALLEL_SEL = 0 (serial TS), then this configuration will 
            have no effect.
            
            Value: 
            - 0: Disable TS smoothing buffer (DVB-T/C).
                 TS clock rate = Fixed 10.25MHz.
            - 1: Enable TS smoothing buffer (Default).
                TS clock rate = auto rate or DEMOD_CONFIG_TSIF_SDPR.
    ...
    */
    ...
    /**
     @brief Enable/Disable the DVB-T2 PP2 Long Echo Sequence.

            Value:
            - 0: Disable DVB-T2 PP2 Long Echo acquisition sequence (Default).
            - 1: Enable DVB-T2 PP2 Long Echo acquisition sequence.
    */
    DEMOD_CONFIG_DVBT2_PP2_LONG_ECHO,
...
/*------------------------------------------------------------------------------
  DVB-T2 PP2 Long Echo acquisition sequencer state, structure and 
...
/**
 @brief PP2 Long Echo acquisition sequencer item definitions.
*/
typedef enum {
    SONY_DVB_LE_SEQ_INIT = 0,               /**< See ::dvb_demod_le_SeqInit. */
    SONY_DVB_LE_SEQ_START,                  /**< See ::dvb_demod_le_SeqStart. */
    SONY_DVB_LE_SEQ_WAIT_P1,                /**< See ::dvb_demod_le_SeqWaitP1. */
    SONY_DVB_LE_SEQ_WAIT_LOCK,              /**< See ::dvb_demod_le_SeqWaitDmdLock. */
    SONY_DVB_LE_SEQ_CRL_OPEN_TRIAL,         /**< See ::dvb_demod_le_SeqCrlOpenTrial. */
    SONY_DVB_LE_SEQ_TIMING_TRIAL,           /**< See ::dvb_demod_le_SeqTimingTrial. */
    SONY_DVB_LE_SEQ_SPECTRUM_TRIAL,         /**< See ::dvb_demod_le_SeqSpectrumTrial. */
    SONY_DVB_LE_SEQ_UNKNOWN                 /**< Unknown sequencer item (also defines max sequencer items). */
} sony_dvb_le_seq_state_t ;

/*------------------------------------------------------------------------------
  PP2 Long Echo Structs
------------------------------------------------------------------------------*/

/**
 @brief DVB-T2 PP2 Long Echo acquisition sequencer for demodulator lock.
*/
typedef struct sony_dvb_le_seq_t {
    sony_dvb_demod_t* pDemod ;              /**< Demodulator instance for sequence. */
    sony_dvb_le_seq_state_t state ;         /**< Current sequencer state. */
    uint8_t autoSpectrumDetect ;            /**< Auto spectrum detection enabled flag. */
    uint32_t pollInterval ;                 /**< Polling interval (ms). */
    uint32_t waitTime ;                     /**< Current wait time (ms) from ::SONY_DVB_LE_SEQ_START. */
    uint32_t totalWaitTime ;                /**< Total wait time (ms). */
    uint8_t spectTrial ;                    /**< Spectrum trial flag. */
    uint8_t gThetaPOfst ;                   /**< Positive GTHETA_OFST trial flag. */
    uint8_t gThetaMOfst ;                   /**< Minus GTHETA_OFST trial flag. */
    uint8_t gThetaNoOfst ;                  /**< No GTHETA_OFST trial flag. */
    uint8_t unlockDetDone ;                 /**< Unlock detection done flag. */
    uint8_t crlOpenTrial ;                  /**< CRL open trial flag. */
    uint8_t complete ;                      /**< Sequencer complete flag. */

    /**
     @brief Sleep function for sequencer.
     
     @param pSeq Sequencer instance.
     @param ms The ms to sleep.
    */
    void (*Sleep) (struct sony_dvb_le_seq_t* pSeq, uint32_t ms);

} sony_dvb_le_seq_t ;

/**
 @brief DVB-T2 PP2 Long Echo acquisition sequencer for TS lock.
*/
typedef struct sony_dvb_le_seq_ts_lock_t {
    sony_dvb_demod_t* pDemod ;              /**< Demodulator instance for sequence. */
    uint32_t pollInterval ;                 /**< Polling interval (ms). */
    uint32_t waitTime ;                     /**< Current wait time (ms). */
    uint8_t complete ;                      /**< Sequencer complete flag. */
    ...
    /**
     ...
     @param ms The ms to sleep.
    */
    void (*Sleep) (struct sony_dvb_le_seq_ts_lock_t* pSeq, uint32_t ms);

} sony_dvb_le_seq_ts_lock_t ;

/*------------------------------------------------------------------------------
  DVB-T2 PP2 Long Echo Sequencer Crank Functions
------------------------------------------------------------------------------*/

/**
 @brief DVB-T2 PP2 Long Echo sequence demodulator lock sequencer crank.
...
/**
 @brief DVB-T2 PP2 Long Echo sequence TS lock sequencer crank.
