...
int i2c_feusb_WriteGw (HANDLE handle, uint8_t deviceAddress, const uint8_t * pData, uint32_t numberOfBytes, uint8_t gwAddress, uint8_t gwSub)
{
    BOOL bResult = FALSE;
    DWORD dwByteReturned = 0;
    PARAM_VENDOR_REQUEST *pVendorReq = NULL;
...
int i2c_feusb_ReadRegister (HANDLE handle, uint8_t deviceAddress, uint8_t subAddress, uint8_t* pData, uint32_t numberOfBytes)
{
    ...
    sVendorReq.Index = (USHORT) (((deviceAddress & 0xff) << 8)
                                 | (subAddress & 0xff)) ;
    while (nLengthRemain > 0) {
